...
exports.restrictTo =
  ...
  (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('You have no permission to perform that action', 403)
      );
    }
    next();
  };
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError(
        'There is a problem with sending email. Try again later',
        500
      )
    );
  }
